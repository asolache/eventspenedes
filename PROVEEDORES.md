# Red de proveedores y partners

Lista de trabajo, **no publicada en la web**. Un proveedor pasa a `index.html`
solo cuando está en estado `Confirmado` y ha dado permiso para aparecer con su
nombre.

Campos de cada ficha: qué aporta · contacto · web · capacidad o rango ·
precio orientativo · estado (`Pendiente` / `Hablado` / `Confirmado`).

---

## 1 · Espacios propios

| Espacio              | Qué es                                   | Estado     |
|----------------------|------------------------------------------|------------|
| **Cal Segue**       | Barri Cusconar, 08737 Torrelles de Foix. Solo espacio exterior: era, porche con barbacoa y jardín | Confirmado, con fotos y aforo de unas 30 personas |
| **Taller de Castells** | Espacio cubierto de entreno casteller (TeamTowers) | Confirmado |

## 2 · Espacios aliados

| Espacio                | Dónde      | Qué aporta                              | Estado |
|------------------------|------------|-----------------------------------------|--------|
| **Masia Torreblanca**  | Olèrdola   | Masía entre viñedos, interior y exterior | Publicado en la web — falta confirmar aforo, contacto y permiso de uso del nombre |

## 3 · Bodegas y catas

| Proveedor              | Qué aporta                 | Contacto | Estado |
|------------------------|----------------------------|----------|--------|
| **La Bodega de Sara** | Catas guiadas de vino y cava, en bodega o desplazadas | labodegadesara.com | Publicada en la web, con foto |

## 4 · Alojamiento y espacios de gama alta

| Proveedor              | Qué aporta                 | Contacto | Estado |
|------------------------|----------------------------|----------|--------|
| **L'Algar** *(grafía por confirmar)* | Espacio de calidad para cliente de lujo, muy cercano | — | Pendiente |
|                        | Hoteles boutique de la comarca |      | Pendiente |
|                        | Casas rurales               |          | Pendiente |

## 5 · Restauración

| Proveedor | Qué aporta                          | Contacto | Estado |
|-----------|-------------------------------------|----------|--------|
|           | Restaurantes de alta gama           |          | Pendiente |
|           | Catering para masía                 |          | Pendiente |
|           | Chef para reto gastronómico         |          | Pendiente |

## 6 · Movilidad y material

| Proveedor | Qué aporta                              | Contacto | Estado |
|-----------|-----------------------------------------|----------|--------|
|           | Vehículos de alta gama con chófer       |          | Pendiente |
|           | Autocar / transfer de grupo             |          | Pendiente |
|           | Alquiler de bicicletas y e-bikes        |          | Pendiente |
|           | Mobiliario, carpas y material técnico   |          | Pendiente |

## 7 · Actividades

| Proveedor      | Qué aporta                                  | Estado     |
|----------------|---------------------------------------------|------------|
| **TeamTowers** | Taller de castells «Fent Pinya» — teamtowers.eu | Confirmado |
|                | Gincana en viñedos                          | Pendiente  |
|                | Guías de naturaleza / rutas                 | Pendiente  |

---

## Qué falta para poder publicar cada uno

1. Nombre comercial exacto y web.
2. Persona de contacto y teléfono o correo.
3. Permiso explícito para aparecer en eventspenedes.com con su nombre y, si
   procede, su logotipo.
4. Capacidad, rango de precio y temporada.
5. Una o dos fotos con derechos de uso.
